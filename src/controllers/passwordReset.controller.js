...
const passwordResetController = {
  ...
  requestVerificationCode: async (req, res, next) => {
    try {
      ...
      if (accountType === 'user' || accountType === 'employee') {
        userData = await UserRepository.findOneByEmail(email);
        if (userData) name = userData.name;
      } else if (accountType === 'company') {
        userData = await CompanyRepository.findByEmail(email);
        if (userData) name = userData.company_name;
      }
  ...
  resetPassword: async (req, res, next) => {
    try {
      const { email, password, accountType, resetToken } = req.body;
      ...
      if (accountType === 'user' || accountType === 'employee') {
        ...
        await CompanyRepository.update(company.company_id, { password_hash });
        updated = true;
      }
      ...
      if (updated) {
        ...
        logger.info(`Password successfully reset for ${email} (${accountType})`);

...
let userController = {
  ...
  find: async (req, res, next) => {
    try {
      ...
      const user = await UserRepository.findByIdExcludeProps(id, [
        "password_hash",
      ]);
  ...
  update: async (req, res, next) => {
    try {
      ...
      // Get updated user
      const updatedUser = await UserRepository.findByIdExcludeProps(id, [
        "password_hash",
      ]);
  ...
  getAllUsers: async (req, res, next) => {
    try {
      ...
      const { count, rows } = await UserRepository.findAllPaginatedUsers(
        page,
        limit
      );
  ...
  getUser: async (req, res, next) => {
    try {
      ...
      const user = await UserRepository.findByIdExcludeProps(userId, [
        "password_hash",
      ]);
  ...
  uploadProfilePicture: async (req, res, next) => {
    try {
      ...
      if (req.userId !== parseInt(userId) && !req.adminEmployee) {
        throw new ForbiddenError(
          "You do not have permission to upload a profile picture for this user"
        );
      }
      ...
      const profilePictureUrl = getRelativePath(
        req.file.path,
        "profile-pictures"
      );
  ...
  deleteProfilePicture: async (req, res, next) => {
    try {
      ...
      if (req.userId !== parseInt(userId) && !req.adminEmployee) {
        throw new ForbiddenError(
          "You do not have permission to delete this profile picture"
        );
      }

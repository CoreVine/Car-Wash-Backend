...
const cartController = {
  ...
  getCarts: async (req, res, next) => {
    try {
      ...
      const { count, rows } = await CartRepository.getUserCarts(req.user.user_id, page, limit);
  ...
  addToCart: async (req, res, next) => {
    try {
      const { item_type, product_id, quantity, car_id, is_rental, 
        wash_types, within_company, location, company_id } = req.body;
      ...
      switch (item_type) {
        ...
        case 'car':
          ...
          if (cart.rentalOrder || cart.carOrder) {
            throw new BadRequestError('A car is already in your cart. Please checkout or remove it first.');
          }
          
          if (is_rental) {
            if (!req.body.start_date || !req.body.end_date) {
              throw new BadRequestError('Start date and end date are required for car rentals');
            }
            ...
            try {
              ...
              throw new BadRequestError(`Failed to add rental car: ${error.message}`);
            }
          } else {
            try {
        ...
        case 'wash_service':
          ...
          if (cart.carWashOrder) {
            throw new BadRequestError('A car wash service is already in your cart. Please checkout or remove it first.');
          }
          ...
          try {
            ...
            throw new BadRequestError(`Failed to add wash service: ${error.message}`);
          }
          break;
          
        default:
          throw new BadRequestError('Invalid item type. Must be "product", "car", or "wash_service"');
      }
      
      // Get updated cart with all associations
      cart = await CartRepository.findCartWithItems(cart.order_id);
      
      return res.success(`${item_type.charAt(0).toUpperCase() + item_type.slice(1)} added to cart`, cart);
    } catch (error) {
      next(error);
    }
  },
      ...
      switch (item_type) {
        ...
        case 'rental_car':
          ...
          if (rentalOrder.order_id !== cart.order_id) {
            throw new BadRequestError('Rental car does not belong to your active cart');
          }
        ...
        case 'sale_car':
          ...
          if (carOrder.orders_order_id !== cart.order_id) {
            throw new BadRequestError('Car does not belong to your active cart');
          }
        ...
        case 'wash_service':
          ...
          const washOrder = await CarWashOrderRepository.findById(wash_order_id);
          ...
          if (washOrder.order_id !== cart.order_id) {
            throw new BadRequestError('Car wash service does not belong to your active cart');
          }
        ...
        case 'wash_type':
          if (!wash_order_id || !wash_type_id) {
            throw new BadRequestError('Both wash order ID and wash type ID are required');
          }
          
          // Verify the wash order exists and belongs to this cart
          const washOrderForType = await CarWashOrderRepository.findById(wash_order_id);
          ...
          await CarWashOrderRepository.removeWashType(wash_order_id, wash_type_id);
          break;
          
        default:
          throw new BadRequestError('Invalid item type. Must be "product", "rental_car", "sale_car", "wash_service", or "wash_type"');
      }

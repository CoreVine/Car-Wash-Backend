...
const authController = {
    ...
    try {
      ...
      if (accountType === "company") {
        ...
        if (validationErrors.length) {
          logger.warn("Company registration validation failed", {
        ...
        const companyExists =
          await CompanyRepository.findCompanyDetailsByEmailOrName(
            email,
            company_name
          );
        ...
        if (jwtResponse.refreshToken) {
          ...
          return res.success("Company registered successfully", {
            company,
            token,
        ...
        const validationErrors = await userSchema
          .validate(req.body, { abortEarly: false })
          .catch((err) => err.errors);
  ...
  login: async (req, res, next) => {
    try {
      ...
      if (accountType === "company") {
        ...
        if (!company.approved) {
          logger.warn(
            `Login failed - company account pending approval: ${email}`
          );
        ...
        logger.info(`Login successful for company: ${email}`, {
          company_id: company.company_id,
        });
        ...
        if (user.acc_type === "employee") {
          ...
          logger.info(`Employee data retrieved for user: ${email}`, {
            user_id: user.user_id,
          });
        }
  ...
  me: async (req, res, next) => {
    try {
      ...
      const user = await UserRepository.findByIdExcludeProps(req.userId, [
        "password_hash",
      ]);

      if (user) {
        // Check if the user is an employee
        logger.info(`${user.acc_type} profile retrieved`, {
          user_id: user.user_id,
        });
        if (user.acc_type === "employee") {
          const employeeData = await EmployeeRepository.findByUserId(
            user.user_id
          );
      ...
      const company = await CompanyRepository.findCompanyWithAllDetails(
        req.userId
      );

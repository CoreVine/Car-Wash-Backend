...
const companyController = {
  getAllCompanies: async (req, res, next) => {
    try {
      ...
      const { count, rows } =
        await CompanyRepository.findCompaniesWithApprovalStatus(
          approved,
          page,
          limit
        );
  ...
  getCompanyWithWashTypes: async (req, res, next) => {
    try {
      ...
      const company = await CompanyRepository.findCompanyBasicDetails(
        companyId
      );
      ...
      return res.success(
        "Company with wash types retrieved successfully",
        result
      );
    } catch (error) {
      next(error);
  ...
  uploadDocument: async (req, res, next) => {
    try {
      ...
      // if (req.userId !== parseInt(companyId) && !req.adminEmployee) {
      //   throw new ForbiddenError(
      //     "You do not have permission to upload documents for this company"
      //   );
      // }
      ...
      const relativePath =
        req.file.url || getRelativePath(req.file.path, "company-documents");
  ...
  getDocuments: async (req, res, next) => {
    try {
      ...
      const { count, rows } =
        await CompanyDocumentRepository.findCompanyDocumentsPaginated(
          companyId,
          page,
          limit
        );

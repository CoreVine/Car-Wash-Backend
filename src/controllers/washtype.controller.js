...
const washTypeController = {
  ...
  getAllWashTypes: async (req, res, next) => {
    try {
      ...
      return res.success('Wash types retrieved successfully', result.rows, pagination);
    } catch (error) {
      next(error);
    }
  },
  ...
  getCompanyWashTypes: async (req, res, next) => {
    try {
      ...
      const { count, rows } = await WashTypeRepository.findByCompanyPaginated(companyId, page, limit);
      
      const pagination = createPagination(page, limit, count);
      
      return res.success('Company wash types retrieved successfully', rows, pagination);
    } catch (error) {
      next(error);
    }
  },
  ...
  updateWashType: async (req, res, next) => {
    try {
      ...
      if (washType.company_id !== req.company.company_id) {
        throw new ForbiddenError('You do not have permission to update this wash type');
      }
  ...
  deleteWashType: async (req, res, next) => {
    try {
      ...
      // Check if wash type belongs to this company
      if (washType.company_id !== req.company.company_id) {
        throw new ForbiddenError('You do not have permission to delete this wash type');
      }
      ...
      if (washOrders.length > 0) {
        throw new BadRequestError('Cannot delete wash type that is being used in orders');
      }
  ...
  getPopularWashTypes: async (req, res, next) => {
    try {
      ...
      const popularWashTypes = await WashTypeRepository.findPopularWashTypes(limit);
      
      return res.success('Popular wash types retrieved successfully', popularWashTypes);
    } catch (error) {
      next(error);
    }
  }
